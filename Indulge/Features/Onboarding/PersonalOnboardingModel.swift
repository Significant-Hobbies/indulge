...
enum PersonalOnboardingStep: Int, CaseIterable, Sendable {
  ...
  static let mainJourney = coreJourney

  var isOptional: Bool {
    switch self {
    case .name, .commonMoment, .startingPattern, .intentionality, .changePace:
      true
    case .gender, .activities, .primaryIndulgence, .timeSpent, .underlyingNeed, .lifeDirection,
      .reflection:
      false
    }
  }
  ...
  func title(for profile: OnboardingProfile) -> String {
    ...
    case .underlyingNeed:
      profile.primaryIndulgence?.purposeQuestion ?? "What are you looking for in that moment?"
  ...
  func body(for profile: OnboardingProfile) -> String {
    ...
    case .activities:
      ""
    case .primaryIndulgence:
      "Choose the one that most often stretches beyond the moment."
    case .timeSpent:
...
enum IndulgenceChoice: String, CaseIterable, Codable, Hashable, Sendable {
  ...
  var purposeQuestion: String {
    ...
    case .shortVideo, .socialFeeds, .newsScroll:
      "When you start scrolling, what are you looking for?"
    ...
    case .snacking, .takeaway, .sweets, .coffee, .alcohol:
      "When you reach for it, what are you looking for?"
  ...
  var isPhoneCompanion: Bool {
    ...
    case .shortVideo, .socialFeeds, .newsScroll, .webBrowsing, .rabbitHoles,
      .takeaway, .onlineShopping, .lyingIn, .texting, .videoCalls:
      true
    default: false
    }
  }
...
struct OnboardingProfile: Codable, Equatable, Sendable {
  ...
  @discardableResult
  mutating func toggleActivity(_ activity: IndulgenceChoice, limit: Int = 5) -> Bool {
  ...
  func canAdvance(from step: PersonalOnboardingStep) -> Bool {
    return switch step {
    case .name, .commonMoment, .startingPattern, .intentionality, .changePace, .reflection:
      true
    case .gender:
      if gender == .selfDescribe {
        !customGender.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
      } else {
        gender != nil
      }
    case .activities:
  ...
  var reflectionBody: String {
    ...
    var sentences = [
      "You enjoy \(activity)\(dailyTime.map { ", and it can hold \($0.reflectionText) of an ordinary day" } ?? "")."
    ]
    ...
    if !lifeDirections.isEmpty {
      sentences.append(
        "You want more room for \(lifeDirectionSummary), without turning pleasure into punishment.")
    }
  ...
  var hasExplicitCharacterPresentation: Bool {
  ...
  func sceneAssetName(for presentation: CharacterPresentation) -> String {
    ...
    case .watchingTelevision(let phone, let drink):
      if phone && drink {
        ...
        "SceneTelevisionPhone\(identity)"
      } else if drink {
        "SceneTelevisionDrink\(identity)"
      } else {
        "SceneTelevision\(identity)"
      }
    case .scrolling:
...
enum OnboardingSceneTransition: Equatable, Sendable {
  ...
  static func resolve(from oldState: OnboardingVisualState, to newState: OnboardingVisualState)
    -> Self
  {
    ...
    case (
      .watchingTelevision(let oldPhone, let oldDrink),
      .watchingTelevision(let newPhone, let newDrink)
    ):
...
extension OnboardingVisualState {
  fileprivate var isWatchingTelevision: Bool {
...
enum PersonalOnboardingPreset: String, Sendable, Equatable {
  ...
  private static let allCasesForLaunch: [PersonalOnboardingPreset] = [
    .name, .keyboard, .gender, .activities, .activitiesMale, .scrolling, .browsing, .gaming,
    .listening, .taste, .resting, .social, .time, .moments, .purpose, .life, .reflection, .reduced,
    .sceneDemo,
  ]
  ...
  var step: PersonalOnboardingStep {
    ...
    case .gender: .gender
    case .activities, .activitiesMale, .scrolling, .browsing, .gaming, .listening, .taste, .resting,
      .social, .sceneDemo:
      .activities
    case .time: .timeSpent
  ...
  var focusesTextEntry: Bool {
    self == .welcome || self == .name || self == .keyboard
  }
  ...
  var profile: OnboardingProfile {
    ...
    if self == .gender { return OnboardingProfile(preferredName: "Maya") }
